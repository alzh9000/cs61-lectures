...
void* benchmark_thread(void* user_data) {
    answer += memnode_benchmark(noperations, (int) (size_t) user_data);
    return nullptr;
}

int main(int argc, char** argv) {
    noperations = 20000000;
    int nthreads = 1;
  ...
  Default NOPS %u, NTHREADS 1, no MEMLIMIT", argv[0], noperations);
    ...
    if (nthreads <= 1) {
        // Run the benchmark.
        answer = memnode_benchmark(noperations, 0);
    } else {
    ...
    double t1 = cputime();
    printf("nops %u, nthreads %d, answer %lu in %.09f sec\n",
           noperations, nthreads, answer.load(), t1 - t0);
}
